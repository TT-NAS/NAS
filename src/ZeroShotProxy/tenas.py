...
ACTIVATIONS = (nn.ReLU, nn.Linear, nn.Softplus, nn.ELU, nn.SELU, nn.Sigmoid, nn.Tanh, nn.Softsign, nn.Softmax)
...
class Linear_Region_Collector:
    ...
    def forward_batch_sample(self):
        for _ in range(self.sample_batch):
            ...
            #     self.loader = iter(self.train_loader)
            #     inputs, targets = self.loader.next()
            inputs = torch.randn(self.input_size, device=CUDA)
...
def get_ntk_n(networks,  data_loader, recalbn=0, train_mode=False):
    ...
    for i, (images, masks) in enumerate(data_loader.train):
        ...
        for net_idx, network in enumerate(networks):
            network.zero_grad()
            inputs_ = images.clone().cuda(device=CUDA, non_blocking=True)
...
def compute_NTK_score(model, data_loader):
    ntk_score = get_ntk_n([model], recalbn=0, train_mode=True, data_loader=data_loader)[0]
    return -1 * ntk_score