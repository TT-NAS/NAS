# Entrena redes neuronales para la obtención de épocas máximas de entrenamiento
import os
import math
...
from src.utils import RESULTS_PATH, IMAGES_PATH
from src.utils import OutOfMemoryError, TorchDataLoader
from src.utils import empty_cache_torch
from src.codec import Chromosome, MAX_LAYERS, MAX_CONVS_PER_LAYER


RESULTS_FILE = os.path.join(RESULTS_PATH, "/max_epochs/results.csv")
LOG_FILE = os.path.join(RESULTS_PATH, "/max_epochs/log.txt")
LEARNING_CURVES_PATH = os.path.join(RESULTS_PATH, "/max_epochs")

def plot_learning_curves(metrics: dict[str, list[float]], save: bool = False,
                         name: str = "learning curves.png", path: str = LEARNING_CURVES_PATH):
    """
